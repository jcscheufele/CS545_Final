...
class BasicNetwork(nn.Module):
  def __init__(self, out_features):
    ...
    self.convolution = nn.Sequential(OrderedDict([
        ("Input", nn.Conv2d(4, 8, kernel_size=40, padding=1)),
        ("ReLU 1", nn.LeakyReLU()),
        ("Hidden 1", nn.Conv2d(8, 32, kernel_size=25, padding=1)),
        ("ReLU 2", nn.LeakyReLU()),
        ("Max Pooling 2", nn.MaxPool2d(2, 2)),

        ("Hidden 2", nn.Conv2d(32, 64, kernel_size=15, padding=1)),
        ("ReLU 3", nn.LeakyReLU()),
        ("Hidden 3", nn.Conv2d(64, 128, kernel_size=15, padding=1)),
        ("ReLU 4", nn.LeakyReLU()),
        ("Max Pooling 4", nn.MaxPool2d(2, 2)),

        ("Hidden 4", nn.Conv2d(128, 128, kernel_size=10, padding=1)),
        ("ReLU 5", nn.LeakyReLU()),
        ("Max Pooling 5", nn.MaxPool2d(2, 2)),

        ("Hidden 5", nn.Conv2d(128, 128, kernel_size=5, padding=1)),
        ("ReLU 6", nn.LeakyReLU()),
        ("Max Pooling 6", nn.MaxPool2d(2, 2)),
...
def train_loop(dataloader, model, loss_fn, optimizer, device, epoch, bs, will_save, key):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        pred[:, 2] = torch.add(pred[:,2], 426)
        pred[:, 3] = torch.add(pred[:,3], 240)
        loss = loss_fn(pred[:,0:4], y)
        ...
        if will_save and (batch % 5 == 0):
            ...
            for idx in range:
                #print(X[idx].size())
                img = X[idx][0:3].detach().cpu().numpy()
